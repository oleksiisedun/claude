...
- **Types**: `pyright` in its default `standard` mode (`strict` is too noisy to adopt by default). Use `mypy` or `ty` only if the project already does.
...
- **Docstrings**: add one only when the signature leaves a real question unanswered (intent, a non-obvious invariant, a gotcha); skip it when the name + signature are self-explanatory. Never restate types in the docstring. When a signature or behavior changes, update the docstring in the same edit.
